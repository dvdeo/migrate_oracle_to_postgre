CREATE OR REPLACE PROCEDURE "API20NINSYO".corp_mst_update_mail(
  ...
  -- 更新内容取得カーソル
  cursor hosoku_cur(i_corp_id varchar2) is
  select hosoku_detail
  from
    api20_hosoku_tbl a ,
    (
      select corp_id,max(ins_dt) mins_dt
      from
        api20_hosoku_tbl
      where
        corp_id = i_corp_id and
        hosoku_kbn = 'U' and
        ins_dt <= sysdate
      group by corp_id
    ) b
  ...
  where kubun = 'BO';
     ...
     if corp_cur%FOUND then     -- 企業データ有り？
         ...
         if corp_rec.maddr is not null and corp_rec.other_maddr is null then
           ...
           mail_to := mail_to || ',' || corp_rec.other_maddr;
           mail_cc := null;
         else
           mail_cc := null;
         end if;
         ...
         -- メール文章作成
         mail_msg := '';
         mail_msg := mail_msg || '-- API2.0顧客マスタ更新通知メール --' || chr(10);
         ...
         if i_kbn = 'U' then
           for hosoku_rec in hosoku_cur(i_cid) loop
             mail_msg := mail_msg || '更新内容:' || hosoku_rec.hosoku_detail || chr(10);
           end loop;
         end if;
