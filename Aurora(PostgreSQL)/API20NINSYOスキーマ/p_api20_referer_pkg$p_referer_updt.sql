CREATE OR REPLACE FUNCTION api20ninsyo.p_api20_referer_pkg$p_referer_updt(IN i_kbn TEXT, IN i_uid TEXT, INOUT i_cid TEXT, INOUT i_key_num DOUBLE PRECISION, INOUT i_referer TEXT, INOUT i_service_name TEXT, IN i_pos DOUBLE PRECISION, IN i_cnt DOUBLE PRECISION, OUT o_ins_dt TEXT, OUT o_upd_dt TEXT, OUT o_rtncd TEXT, OUT o_err_msg TEXT, OUT o_cnt DOUBLE PRECISION, IN o_info VARCHAR, IN o_info$function_name VARCHAR)
AS
$BODY$
        ...
        *
        FROM (SELECT
            ...
            FROM (SELECT
                ...
                FROM api20ninsyo.api20_referer_tbl
                WHERE
                CASE i_cid
                    WHEN NULL THEN 1
...
/* クリア */
BEGIN
    PERFORM aws_oracle_ext.array$copy_structure(o_info, o_info$function_name, 'o_info', 'api20ninsyo.p_api20_referer_pkg$p_referer_updt');
    o_rtncd := '0000';
    o_err_msg := NULL;
    o_cnt := 0;
    w_cnt := 0;
    ...
    IF i_kbn = 'S' AND i_key_num IS NULL THEN
        FOR rf_srch_rec IN rf_search (i_cid, i_referer, i_service_name, i_pos, i_cnt) LOOP
            w_cnt := w_cnt + 1;
            PERFORM aws_oracle_ext.array$set_value('o_info[' || w_cnt || ']', 'api20ninsyo.p_api20_referer_pkg$p_referer_updt', CONCAT_WS('', 'cid', ':"', rf_srch_rec.cid, '",', 'key_num', ':"', rf_srch_rec.key_num, '",', 'referer', ':"', rf_srch_rec.referer, '",', 'service_name', ':"', rf_srch_rec.service_name, '",', 'ins_dt', ':"', rf_srch_rec.ins_dt, '",', 'upd_dt', ':"', rf_srch_rec.upd_dt, '"'));
    ...
    ELSE
                ...
                CASE i_kbn
                    ...
                    THEN
                        ...
                        ELSE
                            ...
                            /* 更新日（YYYYMMDDHHMMSS） */;
                            w_cnt := w_cnt + 1;
                            PERFORM aws_oracle_ext.array$set_value('o_info[' || w_cnt || ']', 'api20ninsyo.p_api20_referer_pkg$p_referer_updt', CONCAT_WS('', 'cid', ':"', rf_rec.cid, '",', 'key_num', ':"', rf_rec.key_num, '",', 'referer', ':"', rf_rec.referer, '",', 'service_name', ':"', rf_rec.service_name, '",', 'ins_dt', ':"', rf_rec.ins_dt, '",', 'upd_dt', ':"', rf_rec.upd_dt, '"'));
                            o_cnt := 1;
                        END IF;
                        w_rtncd := '0000';
                        o_rtncd := w_rtncd;
                    WHEN 'I' THEN
                        ...
                        VALUES (aws_oracle_ext.SYSDATE(), 'REFERER', 'TABLE', i_kbn, i_uid, CONCAT_WS('', i_cid, ' key_num:', aws_oracle_ext.TO_CHAR(i_key_num)), w_rtncd, 'リファラー有りエラー')
                        /*
                    ...
                    /* APIキーテーブルデータ更新 */
                    THEN
                        UPDATE api20ninsyo.api20_referer_tbl
                        ...
                        o_rtncd := w_rtncd;
                        INSERT INTO api20ninsyo.api20_tbl_updt_log
                        VALUES (aws_oracle_ext.SYSDATE(), 'REFERER', 'TABLE', i_kbn, i_uid, CONCAT_WS('', i_cid, ' key_num:', aws_oracle_ext.TO_CHAR(i_key_num)), w_rtncd, NULL)
                        /*
                        [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                        commit
                        */;
                        ...
                        /* テーブル更新ログ書き込み */;
                        INSERT INTO api20ninsyo.api20_tbl_updt_log
                        VALUES (aws_oracle_ext.SYSDATE(), 'REFERER', 'MVIEW', i_kbn, i_uid, i_cid, w_proc_rtncd, NULL)
                        /*
                        [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                        commit
                        */;
                        ...
                        /* テーブル更新ログ書き込み */;
                        INSERT INTO api20ninsyo.api20_tbl_updt_log
                        VALUES (aws_oracle_ext.SYSDATE(), 'REFERER', 'NOREF', i_kbn, i_uid, i_cid, w_proc_rtncd, NULL)
                        /*
                        [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                        commit
                        */;
            ...
            ELSE
                CASE i_kbn
                    ...
                    WHEN 'U' THEN
                        ...
                        o_rtncd := w_rtncd;
                        INSERT INTO api20ninsyo.api20_tbl_updt_log
                        VALUES (aws_oracle_ext.SYSDATE(), 'REFERER', 'TABLE', i_kbn, i_uid, CONCAT_WS('', i_cid, ' key_num:', aws_oracle_ext.TO_CHAR(i_key_num)), w_rtncd, '該当データ無しエラー')
                        /*
                        [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                        commit
                        */;
        ...
        ELSE
            w_rtncd := '0001';
            INSERT INTO api20ninsyo.api20_tbl_updt_log
            VALUES (aws_oracle_ext.SYSDATE(), 'REFERER', 'TABLE', i_kbn, i_uid, CONCAT_WS('', i_cid, ' key_num:', aws_oracle_ext.TO_CHAR(i_key_num)), w_rtncd, '企業ID無しエラー')
            /*
            [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
            commit
            */;
    ...
    /* テーブル更新ログ書き込み */;
    INSERT INTO api20ninsyo.api20_tbl_updt_log
    VALUES (aws_oracle_ext.SYSDATE(), 'REFERER', NULL, i_kbn, i_uid, i_cid, w_rtncd, '処理完了')
    /*
    [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
    commit
    */;
    o_rtncd := w_rtncd;
    o_err_msg := NULL;
    PERFORM aws_oracle_ext.array$assign('o_info', 'api20ninsyo.p_api20_referer_pkg$p_referer_updt', o_info, o_info$function_name);
    PERFORM aws_oracle_ext.array$clear_procedure('api20ninsyo.p_api20_referer_pkg$p_referer_updt');
    EXCEPTION
        WHEN others THEN
            NULL;
            w_rtncd := '7001'
            ...
            */
            /* テーブル更新ログ書き込み */;
            INSERT INTO api20ninsyo.api20_tbl_updt_log
            VALUES (aws_oracle_ext.SYSDATE(), 'REFERER', NULL, i_kbn, i_uid, CONCAT_WS('', i_cid, ' key_num:', aws_oracle_ext.TO_CHAR(i_key_num)), w_rtncd, w_msg)
            /*
            [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
            commit
            */;
