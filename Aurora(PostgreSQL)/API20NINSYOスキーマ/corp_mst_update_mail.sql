CREATE OR REPLACE FUNCTION api20ninsyo.corp_mst_update_mail(IN i_cid TEXT, IN i_kbn TEXT, IN i_uid TEXT, OUT rtncd DOUBLE PRECISION)
AS
$BODY$
/* ZDC社員マスタ検索カーソル */
DECLARE
    ...
    */
    ...
    cursor corp_cur(i_corp_id varchar2) is
      ...
      from api20_corp_mst c, zdc_uid u
    ...
    */
    ...
    corp_rec record
    /* 更新内容取得カーソル */;
    hosoku_cur CURSOR (i_corp_id TEXT) FOR
    SELECT
        hosoku_detail
        FROM api20ninsyo.api20_hosoku_tbl AS a, (SELECT
            corp_id, MAX(ins_dt) AS mins_dt
            FROM api20ninsyo.api20_hosoku_tbl
            WHERE corp_id = i_corp_id AND hosoku_kbn = 'U' AND ins_dt <= aws_oracle_ext.SYSDATE()
            GROUP BY corp_id) AS b
        WHERE a.corp_id = b.corp_id AND a.ins_dt = b.mins_dt AND a.hosoku_kbn = 'U'
    /* ワーク変数 */;
    ...
    */;
    ...
    THEN
        ...
        ELSIF corp_rec.maddr IS NULL AND corp_rec.other_maddr IS NOT NULL THEN
            mail_to := CONCAT_WS('', mail_to, ',', corp_rec.other_maddr);
            mail_cc := NULL;
        ELSE
            mail_cc := NULL;
        END IF
        ...
        /* メール文章作成 */;
        mail_msg := '';
        mail_msg := CONCAT_WS('', mail_msg, '-- API2.0顧客マスタ更新通知メール --', CHR(10));
        ...
        IF i_kbn = 'U' THEN
            FOR hosoku_rec IN hosoku_cur (i_cid) LOOP
                mail_msg := CONCAT_WS('', mail_msg, '更新内容:', hosoku_rec.hosoku_detail, CHR(10));
            END LOOP;
        END IF;
        mail_msg := CONCAT_WS('', mail_msg, CHR(10));
        mail_msg := CONCAT_WS('', mail_msg, '---------------------------------', CHR(10));
