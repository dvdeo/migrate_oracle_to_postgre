CREATE OR REPLACE FUNCTION api20ninsyo.p_api20_corp_pkg$p_corp_updt(IN i_kbn TEXT, IN i_uid TEXT, INOUT i_cid TEXT, INOUT i_passwd TEXT, INOUT i_cname TEXT, INOUT i_cname_kana TEXT, INOUT i_addr TEXT, INOUT i_post TEXT, INOUT i_tel TEXT, INOUT i_dept TEXT, INOUT i_s_date TEXT, INOUT i_e_date TEXT, IN i_s_date_op TEXT, IN i_e_date_op TEXT, INOUT i_sales_id TEXT, INOUT i_other_maddr TEXT, INOUT i_end_flg TEXT, INOUT i_usage TEXT, INOUT i_name TEXT, INOUT i_version TEXT, INOUT i_rprt_flg TEXT, IN i_pos DOUBLE PRECISION, IN i_cnt DOUBLE PRECISION, OUT o_ins_dt TEXT, OUT o_upd_dt TEXT, OUT o_rtncd TEXT, OUT o_err_msg TEXT, OUT o_cnt DOUBLE PRECISION, IN o_info VARCHAR, IN o_info$function_name VARCHAR)
AS
$BODY$
...
DECLARE
    ...
    */
    ...
    SELECT
        cid, passwd, cname, cname_kana, addr, post, tel, dept, aws_oracle_ext.TO_CHAR(s_date, 'yyyymmddhh24miss') AS s_date, aws_oracle_ext.TO_CHAR(e_date, 'yyyymmddhh24miss') AS e_date, sales_id, other_maddr, end_flg, usage, name, version, rprt_flg, aws_oracle_ext.TO_CHAR(ins_dt, 'yyyymmddhh24miss') AS ins_dt, aws_oracle_ext.TO_CHAR(upd_dt, 'yyyymmddhh24miss') AS upd_dt
...
/* うるう年補正（2/29 -> 2/28） */
BEGIN
    PERFORM aws_oracle_ext.array$copy_structure(o_info, o_info$function_name, 'o_info', 'api20ninsyo.p_api20_corp_pkg$p_corp_updt');
    ...
    IF i_kbn = 'S' AND (i_cid IS NULL OR aws_oracle_ext.substr(i_cid, 1, 1) = '*') THEN
        FOR api_corp_search_rec IN api_corp_search (i_cid, i_passwd, i_cname, i_cname_kana, i_addr, i_post, i_tel, i_dept, i_s_date, i_e_date, i_s_date_op, i_e_date_op, i_sales_id, i_other_maddr, i_end_flg, i_usage, i_name, i_version, i_rprt_flg, i_pos, i_cnt) LOOP
            w_cnt := w_cnt + 1
            /*
                ...
                CASE i_kbn
                    WHEN 'S' THEN
                        ...
                        */;
                        ...
                        w_cnt := w_cnt + 1;
                        PERFORM aws_oracle_ext.array$set_value('o_info[' || w_cnt || ']', 'api20ninsyo.p_api20_corp_pkg$p_corp_updt', CONCAT_WS('', 'cid', ':"', api_corp_rec.cid, '",', 'passwd', ':"', api_corp_rec.passwd, '",', 'cname', ':"', api_corp_rec.cname, '",', 'cname_kana', ':"', api_corp_rec.cname_kana, '",', 'addr', ':"', api_corp_rec.addr, '",', 'post', ':"', api_corp_rec.post, '",', 'tel', ':"', api_corp_rec.tel, '",', 'dept', ':"', api_corp_rec.dept, '",', 's_date', ':"', api_corp_rec.s_date, '",', 'e_date', ':"', api_corp_rec.e_date, '",', 'sales_id', ':"', api_corp_rec.sales_id, '",', 'usermei', ':"', w_usermei, '",', 'other_maddr', ':"', api_corp_rec.other_maddr, '",', 'end_flg', ':"', api_corp_rec.end_flg, '",', 'usage', ':"', api_corp_rec.usage, '",', 'name', ':"', api_corp_rec.name, '",', 'version', ':"', api_corp_rec.version, '",', 'rprt_flg', ':"', api_corp_rec.rprt_flg, '",', 'ins_dt', ':"', api_corp_rec.ins_dt, '",', 'upd_dt', ':"', api_corp_rec.upd_dt, '"'));
                        o_cnt := 1;
                        o_rtncd := '0000'
                        /*
                        [5578 - Severity CRITICAL - Unable to automatically transform the SELECT statement. Try rewriting the statement.]
                    ...
                    WHEN 'I' THEN
                        ...
                        VALUES (aws_oracle_ext.SYSDATE(), 'MST', 'TABLE', i_kbn, i_sales_id, i_cid, w_rtncd, '企業ID有りエラー')
                        /*
                    ...
                    WHEN 'U' THEN
                        ...
                        */
                        ...
                        /* API企業マスタデータ更新 */
                        THEN
                            UPDATE api20ninsyo.api20_corp_mst
                            SET cid = i_cid
                            ...
                            /* 用途 */, name = i_name
                            /* 氏名 */, version = i_version
                            /* APIバージョン */, rprt_flg = COALESCE(i_rprt_flg, '0')
                            /* ログレポート閲覧許可フラグ(0:不可,1:可) */, upd_dt = aws_oracle_ext.SYSDATE()
                            /* 更新日（YYYYMMDDHHMMSS） */
                            ...
                            */;
                            ...
                            /* テーブル更新ログ書き込み */;
                            INSERT INTO api20ninsyo.api20_tbl_updt_log
                            VALUES (aws_oracle_ext.SYSDATE(), 'MST', 'TABLE', i_kbn, i_sales_id, i_cid, w_rtncd, NULL)
                            /*
                            [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                            commit
                            */;
                            ...
                            */;
                            SELECT
                                *
                                FROM api20ninsyo.p_refresh('AUTH_NOREF_MVIEW'::TEXT)
                                INTO w_proc_rtncd
                            /* テーブル更新ログ書き込み */;
                            INSERT INTO api20ninsyo.api20_tbl_updt_log
                            VALUES (aws_oracle_ext.SYSDATE(), 'MST', 'NOREF', i_kbn, i_sales_id, i_cid, w_proc_rtncd, NULL)
                            /*
                            [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                            commit
                            */
                            ...
                            SELECT
                                *
                                FROM api20ninsyo.corp_mst_update_mail(i_cid, i_kbn, i_uid)
                                INTO w_n_rtncd_mail;
                            ...
                            /* テーブル更新ログ書き込み */;
                            INSERT INTO api20ninsyo.api20_tbl_updt_log
                            VALUES (aws_oracle_ext.SYSDATE(), 'MST', 'MAIL', i_kbn, i_uid, i_cid, w_c_rtncd_mail, NULL)
                            /*
                            [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                            commit
                            */;
                        ELSE
                            ...
                            /* テーブル更新ログ書き込み */;
                            INSERT INTO api20ninsyo.api20_tbl_updt_log
                            VALUES (aws_oracle_ext.SYSDATE(), 'MST', 'TABLE', i_kbn, i_sales_id, i_cid, w_rtncd, CONCAT_WS('', 'ユーザーID無しエラー（USERID:', i_uid, ')'))
                            /*
                            [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                            commit
                            */;
                    ...
                    THEN
                        ...
                        commit
                        */;
                        DELETE FROM api20ninsyo.api20_corp_mst
                            WHERE CURRENT OF api_corp_cur
                        /* 同じ企業IDのリファラーも同時に削除 */;
                        DELETE FROM api20ninsyo.api20_referer_tbl
                            WHERE cid = i_cid;
                        w_rtncd := '0000';
                        o_rtncd := w_rtncd
                        /* テーブル更新ログ書き込み */;
                        INSERT INTO api20ninsyo.api20_tbl_updt_log
                        VALUES (aws_oracle_ext.SYSDATE(), 'MST', 'TABLE', i_kbn, i_uid, i_cid, w_rtncd, NULL)
                        /*
                        [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                        commit
                        */;
            ...
            ELSE
                CASE i_kbn
                    ...
                    WHEN 'I' THEN
                        ...
                        */
                        ...
                        THEN
                            ...
                            /* 氏名 */, i_version
                            /* APIバージョン */, COALESCE(i_rprt_flg, '0')
                            /* ログレポート閲覧許可フラグ(0:不可,1:可) */)
                            /*
                            [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                            commit
                    ...
                    WHEN 'U' THEN
                        ...
                        o_rtncd := w_rtncd;
                        INSERT INTO api20ninsyo.api20_tbl_updt_log
                        VALUES (aws_oracle_ext.SYSDATE(), 'MST', 'TABLE', i_kbn, i_sales_id, i_cid, w_rtncd, '企業ID無しエラー')
                        /*
                        [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
                        commit
                        */;
                    WHEN 'D' THEN
                        RAISE DEBUG USING MESSAGE = 'TEST2';
                        w_rtncd := '0001';
                        o_rtncd := w_rtncd;
                        INSERT INTO api20ninsyo.api20_tbl_updt_log
                        VALUES (aws_oracle_ext.SYSDATE(), 'MST', 'TABLE', i_kbn, i_sales_id, i_cid, w_rtncd, '企業ID無しエラー')
    ...
    /* テーブル更新ログ書き込み */;
    INSERT INTO api20ninsyo.api20_tbl_updt_log
    VALUES (aws_oracle_ext.SYSDATE(), 'MST', NULL, i_kbn, i_uid, i_cid, w_rtncd, '処理完了')
    /*
    [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
    commit
    */;
    o_rtncd := w_rtncd;
    o_err_msg := NULL;
    PERFORM aws_oracle_ext.array$assign('o_info', 'api20ninsyo.p_api20_corp_pkg$p_corp_updt', o_info, o_info$function_name);
    PERFORM aws_oracle_ext.array$clear_procedure('api20ninsyo.p_api20_corp_pkg$p_corp_updt');
    EXCEPTION
        WHEN others THEN
            NULL;
            w_rtncd := '7001'
            ...
            */
            /* テーブル更新ログ書き込み */;
            INSERT INTO api20ninsyo.api20_tbl_updt_log
            VALUES (aws_oracle_ext.SYSDATE(), 'MST', NULL, i_kbn, i_uid, i_cid, w_rtncd, w_msg)
            /*
            [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
            commit
            */;
