CREATE OR REPLACE FUNCTION api20ninsyo.p_oauth_pkg$p_hosoku_tbl_updt(IN i_kbn TEXT, IN i_uid TEXT, INOUT i_corp_id TEXT, INOUT i_key_num DOUBLE PRECISION, INOUT i_hosoku_kbn TEXT, INOUT i_hosoku_title TEXT, INOUT i_hosoku_detail TEXT, INOUT i_userid TEXT, IN i_pos DOUBLE PRECISION, IN i_cnt DOUBLE PRECISION, OUT o_ins_dt TEXT, OUT o_upd_dt TEXT, OUT o_rtncd TEXT, OUT o_err_msg TEXT, OUT o_cnt DOUBLE PRECISION, IN o_info VARCHAR, IN o_info$function_name VARCHAR)
AS
$BODY$
DECLARE
    /*
    ...
    */
    ...
    SELECT
        ...
        FROM api20ninsyo.oauth_hosoku_tbl
...
/* クリア */
BEGIN
    PERFORM aws_oracle_ext.array$copy_structure(o_info, o_info$function_name, 'o_info', 'api20ninsyo.p_oauth_pkg$p_hosoku_tbl_updt');
    o_rtncd := '0000';
    w_rtncd := '0000';
    o_err_msg := NULL;
    o_cnt := 0;
                ...
                THEN
                    ...
                    ELSE
                        ...
                        */;
                        ...
                        w_cnt := w_cnt + 1;
                        PERFORM aws_oracle_ext.array$set_value('o_info[' || w_cnt || ']', 'api20ninsyo.p_oauth_pkg$p_hosoku_tbl_updt', CONCAT_WS('', 'corp_id', ':"', hosoku_rec.corp_id, '",', 'key_num', ':"', hosoku_rec.key_num, '",', 'hosoku_kbn', ':"', hosoku_rec.hosoku_kbn, '",', 'hosoku_title', ':"', hosoku_rec.hosoku_title, '",', 'hosoku_detail', ':"', hosoku_rec.hosoku_detail, '",', 'userid', ':"', hosoku_rec.userid, '",', 'usermei', ':"', w_usermei, '",', 'ins_dt', ':"', hosoku_rec.ins_dt, '",', 'upd_dt', ':"', hosoku_rec.upd_dt, '"'));
                        o_cnt := 1;
                    END IF;
                WHEN 'I' THEN
                    w_rtncd := '0002';
                WHEN 'U' THEN
                    UPDATE api20ninsyo.oauth_hosoku_tbl
                    ...
                    w_rtncd := '0000';
                WHEN 'D' THEN
                    DELETE FROM api20ninsyo.oauth_hosoku_tbl
                        WHERE CURRENT OF hosoku_cur;
                    w_rtncd := '0000';
                ELSE
                    w_rtncd := '0099';
        ...
        ELSE
            CASE i_kbn
                ...
                THEN
                    ...
                    END IF;
                    INSERT INTO api20ninsyo.oauth_hosoku_tbl
                    VALUES (i_corp_id
                    /* 企業ID */, i_key_num
                    /* キー */, i_hosoku_kbn
                    /* 補足区分 */, i_hosoku_title
    ...
    VALUES (aws_oracle_ext.SYSDATE(), 'OAUTH_HSK', NULL, i_kbn, i_uid, CONCAT_WS('', i_corp_id, ' ', aws_oracle_ext.TO_CHAR(i_key_num)), w_rtncd, NULL)
    /*
    ...
    */;
    o_rtncd := w_rtncd;
    o_err_msg := NULL;
    PERFORM aws_oracle_ext.array$assign('o_info', 'api20ninsyo.p_oauth_pkg$p_hosoku_tbl_updt', o_info, o_info$function_name);
    PERFORM aws_oracle_ext.array$clear_procedure('api20ninsyo.p_oauth_pkg$p_hosoku_tbl_updt');
    EXCEPTION
        WHEN others THEN
            NULL;
            w_rtncd := '7001'
            ...
            */
            /* テーブル更新ログ書き込み */;
            INSERT INTO api20ninsyo.api20_tbl_updt_log
            VALUES (aws_oracle_ext.SYSDATE(), 'OAUTH_HSK', NULL, i_kbn, i_uid, CONCAT_WS('', i_corp_id, ' ', aws_oracle_ext.TO_CHAR(i_key_num)), w_rtncd, w_msg)
            /*
            [5350 - Severity CRITICAL - Unable automatically convert statements that explicitly apply or cancel a transaction. Revise your code to try move transaction control on side of application.]
            commit
            */;
